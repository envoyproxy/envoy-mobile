...
TEST(TestSendHeaders, CanSendHeaders) {
  ...
  stream_prototype->set_on_headers([&](Platform::ResponseHeadersSharedPtr headers, bool end_stream) {
  ...
  Platform::RequestHeadersBuilder request_headers_builder(Platform::RequestMethod::GET, "https", "example.com", "/");
  auto request_headers = request_headers_builder.build();
  auto request_headers_ptr = Platform::RequestHeadersSharedPtr(new Platform::RequestHeaders(request_headers));
