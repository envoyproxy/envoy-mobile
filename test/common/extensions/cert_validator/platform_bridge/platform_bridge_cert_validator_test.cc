...
class PlatformBridgeCertValidatorTest : public testing::TestWithParam<CertificateValidationContext::TrustChainVerification> {
protected:
  PlatformBridgeCertValidatorTest()
    : api_(Api::createApiForTest()),
      dispatcher_(api_->allocateDispatcher("test_thread")),
      stats_(generateSslStats(test_store_)),
      ssl_ctx_(SSL_CTX_new(TLS_method())),
      callback_(std::make_unique<MockValidateResultCallback>()),
      is_server_(false) {
  ...
  static envoy_cert_validation_result validate(const envoy_data* certs, uint8_t size, const char* hostname) {
...
std::thread::id PlatformBridgeCertValidatorTest::main_thread_id_;
  ...
  INSTANTIATE_TEST_SUITE_P(TrustMode, PlatformBridgeCertValidatorTest, testing::ValuesIn({CertificateValidationContext::VERIFY_TRUST_CHAIN,
	CertificateValidationContext::ACCEPT_UNTRUSTED}));

TEST_P(PlatformBridgeCertValidatorTest, NoConfig) {
  EXPECT_ENVOY_BUG({ PlatformBridgeCertValidator validator(nullptr, stats_, &platform_validator_);} ,
...
TEST_P(PlatformBridgeCertValidatorTest, NonEmptyCaCert) {
  ...
  EXPECT_ENVOY_BUG({ PlatformBridgeCertValidator validator(&config_, stats_, &platform_validator_);} ,
		   "Invalid certificate validation context config.");
}
...
TEST_P(PlatformBridgeCertValidatorTest, NoCallback) {
  ...
  EXPECT_ENVOY_BUG({
    validator.doVerifyCertChain(*cert_chain,
...
TEST_P(PlatformBridgeCertValidatorTest, ValidCertificate) {
  ...
  EXPECT_CALL(*mock_validator_, cleanup()).Times(1);
...
TEST_P(PlatformBridgeCertValidatorTest, ValidCertificateButInvalidSni) {
  ...
  EXPECT_CALL(callback_ref, onCertValidationResult(acceptInvalidCertificates(), "PlatformBridgeCertValidator_verifySubjectAltName failed: SNI mismatch.", SSL_AD_BAD_CERTIFICATE)).WillOnce(Invoke([this]() {
    dispatcher_->exit();
  }));
  EXPECT_FALSE(waitForDispatcherToExit());
}
...
TEST_P(PlatformBridgeCertValidatorTest, ValidCertificateSniOverride) {
  ...
  transport_socket_options_ = std::make_shared<Network::TransportSocketOptionsImpl>("", std::move(subject_alt_names));
