...
TEST_F(DispatcherTest, BasicStreamHeadersOnly) {
  ...
  observer.on_headers = [](envoy_headers c_headers, bool end_stream, void* context) -> void {
    ...
    HeaderMapPtr response_headers = Utility::toCppHeaders(c_headers);
  ...
  envoy_headers c_headers = Utility::toCHeaders(headers);
...
TEST_F(DispatcherTest, MultipleStreams) {
  ...
  envoy_headers c_headers2 = Utility::toCHeaders(headers2);
