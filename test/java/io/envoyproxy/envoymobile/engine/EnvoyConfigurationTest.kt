...
class EnvoyConfigurationTest {
  ...
  fun `resolving with default configuration resolves with values`() {
    val envoyConfiguration = EnvoyConfiguration(
      "stats.foo.com", null, 123, 234, 345, 456, 567, 678, "[test]", "v1.2.3", "com.mydomain.myapp", "[test]",
    ...
    assertThat(resolvedTemplate).contains("&dns_preresolve_hostnames [test]")
  ...
  fun `cannot configure both statsD and gRPC stat sink`() {
    val envoyConfiguration = EnvoyConfiguration(
      "stats.foo.com", 5050, 123, 234, 345, 456, 567, 678, "[test]", "v1.2.3", "com.mydomain.myapp", "[test]",
      emptyList(), emptyList(), emptyMap()
    )
