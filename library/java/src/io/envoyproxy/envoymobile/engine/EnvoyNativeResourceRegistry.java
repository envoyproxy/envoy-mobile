...
/**
 * Central class to manage releasing native resources when wrapper objects are flagged as unreachable by the garbage collector.
 */
public enum EnvoyNativeResourceRegistry {
  SINGLETON;

  private ReferenceQueue refQueue; // References are automatically enqueued when the gc flags them as unreachable.
  private Set refMaintainer; // Maintains references in the object graph while we wait for them to be enqueued.
  private RefQueueThread refQueueThread; // Blocks on the reference queue and calls the releaser of queued references.

  private class RefQueueThread extends Thread {
    public void run() {
      EnvoyPhantomRef ref;
      while (true) {
        try { ref = (EnvoyPhantomRef) refQueue.remove(); }
