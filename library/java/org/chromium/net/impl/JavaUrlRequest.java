...
final class JavaUrlRequest extends UrlRequestBase {
  ...
  private static final class SerializingExecutor implements Executor {
    ...
    @Override
    public void execute(Runnable command){synchronized (mTaskQueue){mTaskQueue.addLast(command);
    try {
      mUnderlyingExecutor.execute(mRunTasks);
    } catch (RejectedExecutionException e) {
 ...
 */
private void transitionStates(@State int expected, @State int newState, Runnable afterTransition) {
...
private void fireGetHeaders() {
  ...
  mExecutor.execute(errorSetting(new CheckedRunnable() {
    ...
    public void run() throws Exception {
      ...
      String headerKey;
      for (int i = 0; (headerKey = mCurrentUrlConnection.getHeaderFieldKey(i)) != null; i++) {
        ...
        if (!headerKey.startsWith(X_ANDROID)) {
          headerList.add(new SimpleEntry<>(headerKey, mCurrentUrlConnection.getHeaderField(i)));
        }
      }
