 ...
 */
final class CronvoyEngine extends CronvoyEngineBase {
    ...
    engine = new AndroidEngineBuilder(builder.getContext())
                 .setOnEngineRunning(() -> {
                   Runnable taskToExecuteWhenInitializationIsCompleted =
                       initializationCompleter.getAndSet(() -> {});
  ...
  CronvoyEngineBuilderImpl getBuilder() { return builder; }

  void setTaskToExecuteWhenInitializationIsCompleted(Runnable runnable) {
    if (!initializationCompleter.compareAndSet(null, runnable)) {
  ...
  createRequest(String url, UrlRequest.Callback callback, Executor executor, int priority,
                ...
                int idempotency) {
    return new CronvoyUrlRequest(this, callback, executorService, executor, url, userAgent,
