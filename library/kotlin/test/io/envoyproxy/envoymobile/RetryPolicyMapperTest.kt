...
import io.envoyproxy.envoymobile.toHeaders
...
class RetryPolicyMapperTest {
  ...
  fun `all retry policy properties should all be encoded`() {
    ...
    assertThat(retryPolicy.toHeaders()).isEqualTo(mapOf(
  ...
  fun `retry policy without perRetryTimeoutMs should exclude per try time ms header key`() {
    ...
    assertThat(retryPolicy.toHeaders()).doesNotContainKey("x-envoy-upstream-rq-per-try-timeout-ms")
  }
}