...
void Client::DirectStreamCallbacks::encodeHeaders(const ResponseHeaderMap& headers,
  ...
  if (!error_code_header.empty()) {
    ...
    RELEASE_ASSERT(check, fmt::format("[S{}] parse error reading error code", direct_stream_.stream_handle_));
...
void Client::removeStream(envoy_stream_t stream_handle) {
  RELEASE_ASSERT(dispatcher_.isThreadSafe(),
                 fmt::format("[S{}] stream removeStream must be performed on the dispatcher_'s thread.", stream_handle));
  Client::DirectStreamSharedPtr direct_stream = getStream(stream_handle);
  RELEASE_ASSERT(direct_stream,
                 fmt::format("[S{}] removeStream is a private method that is only called with stream ids that exist", stream_handle));
