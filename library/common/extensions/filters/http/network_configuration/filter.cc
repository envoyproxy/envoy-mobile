...
void NetworkConfigurationFilter::setDecoderFilterCallbacks(
  ...
  extra_stream_info_->configuration_key_ = network_connectivity_manager_->addUpstreamSocketOptions(options);
...
Http::LocalErrorStatus NetworkConfigurationFilter::onLocalReply(const LocalReplyData& reply) {
  ...
  // as a network fault. This status is passed to the connectivity_manager below when we report network
