...
Http::FilterFactoryCb NetworkConfigurationFilterFactory::createFilterFactoryFromProtoTyped(
  ...
  auto network_connectivity_manager = Network::ConnectivityManagerFactory{context}.get();
  ...
  return [network_connectivity_manager, enable_drain_post_dns_refresh,
    ...
    callbacks.addStreamFilter(std::make_shared<NetworkConfigurationFilter>(
        network_connectivity_manager, enable_drain_post_dns_refresh, enable_interface_binding));
  };
}
