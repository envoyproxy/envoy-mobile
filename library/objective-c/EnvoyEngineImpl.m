...
envoy_cert_validation_result make_result(envoy_status_t status,
					 uint8_t tls_alert,
...
static envoy_cert_validation_result verify_cert(const envoy_data* certs, uint8_t num_certs,
  ...
  if (!trust_policies) {
    return make_result(ENVOY_FAILURE, SSL_AD_CERTIFICATE_UNKNOWN, "validation couldn't be conducted.");
  }
  ...
  OSStatus status =
      SecTrustCreateWithCertificates(cert_array, trust_policies, &trust);
  if (status) {
    return make_result(ENVOY_FAILURE, SSL_AD_CERTIFICATE_UNKNOWN, "validation couldn't be conducted.");
  }
...
void register_platform_cert_verifier() {
  NSLog(@"[Envoy] registering platform verifier");
...
- (int)runWithTemplate:(NSString *)yaml
  ...
  if (config.enablePlatformCertificateValidation) {
    register_platform_cert_verifier();
  }
